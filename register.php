<?php
	...
	if(isSet($_GET['e'])) {
		...
		if(strpos($err, "fname")) {
			$error = $error . '<div class="error">You must enter a first name</div>';
		}
		if(strpos($err, "lname")) {
			$error = $error . '<div class="error">You must enter a last name</div>';
		}
		if(strpos($err, "email0")) {
			$error = $error . '<div class="error">You must enter an email address</div>';
		}
		if(strpos($err, "email1")) {
			$error = $error . '<div class="error">The email address you provided is not valid</div>';
		}
		if(strpos($err, "username0")) {
			$error = $error . '<div class="error">You must enter a username</div>';
		}
		if(strpos($err, "username1")) {
			$error = $error . '<div class="error">The username you entered is already taken</div>';
		}
		if(strpos($err, "misc")) {
			$error = $error . '<div class="error">A miscellaneous error was encountered while attempting to process the information you provided</div>';
		}
		if(strpos($err, "password0")) {
			$error = $error . '<div class="error">You must enter a password</div>';
		}
		if(strpos($err, "password1")) {
			$error = $error . '<div class="error">Your password and confirmation do not match</div>';
		}
		if(strpos($err, "password2")) {
			$error = $error . '<div class="error">Your password must be greater than 4 characters long</div>';
		}
		if(strpos($err, "cat0")) {
			$error = $error . '<div class="error">You must select either one or two categories that apply to you</div>';
		}
		if(strpos($err, "cat3")) {
			$error = $error . '<div class="error">You must select no more than two categories that apply to you</div>';
		}
		
		if(strpos($err, "phone")) {
			$error = $error . '<div class="error">You must enter a phone number at which you can be contacted</div>';
		}
		if(strpos($err, "address")) {
			$error = $error . '<div class="error">You must enter an address</div>';
		}
		if(strpos($err, "city")) {
			$error = $error . '<div class="error">You must anter a city</div>';
		}
		if(strpos($err, "state")) {
			$error = $error . '<div class="error">You must select a state</div>';
		}
		if(strpos($err, "zipcode")) {
			$error = $error . '<div class="error">You must enter a zipcode</div>';
		}
	}
?>
...
<form action="rec/func/doregistration.php" method="post">
	...
	Full name of parent(s): (Students only)<br>
